...
export const AlumniCenterCardCFF = ({
  ...
  return (
    <div onClick={handleClicked} className="snap-start min-w-[250px] sm:min-w-[280px] md:min-w-[320px] bg-white rounded-2xl shadow-md hover:bg-gray-100 transition duration-300 p-4 flex flex-col justify-between">
      {/* LOGO + BADGE WRAPPER */}
      ...
      <div className="flex gap-2 flex-wrap text-xs font-medium mb-3">
        {fellows.tags.map((tag, index) => (
          <span
            key={index}
            className="bg-orange-100 text-orange-500 px-2 py-1 rounded-full"
          >
      ...
      <div className="text-sm text-gray-700 mb-4 space-y-1">
        ...
        <p className="text-xs text-gray-400">{fellows.dateWhenMakeInformation}</p>
      </div>
...
export const AlumniCenterCardBA = ({
  ...
  return (
    <div onClick={handleClicked} className="snap-start min-w-[250px] sm:min-w-[280px] md:min-w-[320px] bg-white rounded-2xl shadow-md hover:bg-gray-100 transition duration-300 p-4 flex flex-col justify-between">
      ...
      {/* Tags */}
      <div className="flex gap-2 flex-wrap text-xs font-medium mb-3">
        <span className="bg-[#BEC4D5] text-[#0D4690] px-2 py-1 rounded-full">
          Offline
        </span>
      </div>
      ...
      <div className="text-sm text-gray-700 mb-4 space-y-1">
        ...
        <p className="flex items-center gap-2">
          <CalendarDays size="20" /> Batas Pendaftaran: {bonding.applyingDeadline}
        </p>
        <p className="text-xs text-gray-400">{bonding.dateWhenMakeInformation}</p>
      </div>
